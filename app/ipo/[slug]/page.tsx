...
export default async function IPOPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <Link href="/ipo" className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-orange-600 mb-6"><ArrowLeft className="w-4 h-4"/> Back to IPO Dashboard</Link>
      ...
      {ipo.analyst_rating && (
        <div className="mt-6 p-5 bg-white rounded-xl shadow-sm border">
          ...
          <div className={`mt-2 text-2xl font-bold ${ipo.analyst_rating === 'Apply' ? 'text-green-600' : 'text-red-600'}`}>{ipo.analyst_rating}</div>
