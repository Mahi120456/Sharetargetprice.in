      ...
      <section className="mb-12">
        ...
        <div className="overflow-x-auto">
          <table className="w-full bg-white rounded-xl shadow">
            <thead className="bg-gray-50"><tr><th className="p-3 text-left">Company</th><th>Open Date</th><th>Close Date</th><th>Price Band</th><th>Lot Size</th><th>GMP</th><th>Rating</th></tr></thead>
            <tbody>
              {upcoming?.map(ipo => (
                <tr key={ipo.id} className="border-t">
                  ...
                  <td><span className={`px-2 py-1 rounded-full text-xs ${ipo.analyst_rating === 'Apply' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>{ipo.analyst_rating || '-'}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          {current?.map(ipo => (
            <Link key={ipo.id} href={`/ipo/${ipo.slug}`} className="bg-white rounded-xl p-4 shadow hover:shadow-md">
              ...
              <div className="grid grid-cols-2 gap-2 mt-2 text-sm">
                <div>Price: {ipo.price_band}</div><div>Lot: {ipo.lot_size}</div>
                <div>GMP: ₹{ipo.gmp}</div><div>Subscription: {ipo.subscription_total}x</div>
              </div>
            </Link>
          ))}
        </div>
          ...
          <table className="w-full bg-white rounded-xl shadow">
            ...
            <tbody>
              {recent?.map(ipo => {
                ...
                let returnPct = listingPrice && currentPrice ? ((currentPrice - listingPrice)/listingPrice*100).toFixed(2) : null;
                return <tr key={ipo.id} className="border-t">
                  ...
                  <td>{listingPrice || '-'}</td>
                  <td>{currentPrice || '-'}</td>
                  <td className={returnPct >= 0 ? 'text-green-600' : 'text-red-600'}>{returnPct ? `${returnPct}%` : '-'}</td>
                </tr>;
              })}
            </tbody>
          </table>
