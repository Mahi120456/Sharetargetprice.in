...
export async function generateMetadata({ params }: { params: { category: string } }): Promise<Metadata> {
  const { categoryName, funds } = await getCategoryFunds(params.category);
  if (!funds.length) return { title: 'Category Not Found' };
  
  return {
    title: `${categoryName} Mutual Funds - List, Returns, Ratings | ShareTargetPrice`,
    description: `Explore top ${categoryName} mutual funds in India. Compare NAV, AUM, expense ratio, SIP returns and risk ratings. Find the best ${categoryName} funds for your portfolio.`,
    keywords: `${categoryName} mutual funds, best ${categoryName} funds, ${categoryName} fund returns, SIP in ${categoryName}, top ${categoryName} funds India`,
    alternates: { canonical: `https://sharetargetprice.in/mutual-funds/category/${params.category}` },
  };
}
...
export default async function CategoryPage({ params }: { params: { category: string } }) {
  const { funds, categoryName } = await getCategoryFunds(params.category);
      ...
      <div className="container mx-auto px-4 sm:px-6 py-6 sm:py-8 max-w-6xl">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 text-lg max-w-3xl">
            Explore top {categoryName} funds in India. Compare returns, NAV, expense ratio and risk levels to find the best fund for your investment goals.
          </p>
            ...
            >
              <div className="flex justify-between items-start mb-3">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">{fund.fund_house}</p>
                </div>
