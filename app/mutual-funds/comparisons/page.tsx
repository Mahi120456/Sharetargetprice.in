...
import { ArrowLeft, Search, Filter, ChevronDown, TrendingUp, Building2, Tag } from 'lucide-react';

export const revalidate = 3600;

// ---------- Helper to get short slug from fund name ----------
...
// ---------- Fetch all comparison pairs with fund details ----------
async function getAllComparisons() {
  // 1. Get all funds (to build a map from short slug to fund data)
  ...
  // 3. For each slug, split and map to funds
  ...
  }).filter(item => item.fund1 && item.fund2); // ignore any invalid pairs

  return enriched;
}

// ---------- Client Component (for filters + pagination) ----------
'use client';
import { useState, useMemo } from 'react';

function ComparisonsClient({ initialComparisons }: { initialComparisons: any[] }) {
  ...
  // Extract unique categories and AMCs from the data
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50">
      <div className="container mx-auto px-4 sm:px-6 py-6 sm:py-8 max-w-6xl">
        ...
        <Link
          href="/mutual-funds"
          className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-orange-600 transition-colors mb-6 group"
        >
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-gray-600 max-w-2xl mx-auto">
            ...
            Filter by category, AMC, or search by fund name.
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="relative">
              ...
              <select
                value={amc}
                onChange={(e) => { setAmc(e.target.value); setCurrentPage(1); }}
                className="w-full pl-9 pr-8 py-2 border border-gray-200 rounded-lg appearance-none bg-white focus:ring-2 focus:ring-orange-500"
              >
            ...
            {/* Reset Filters */}
            <button
              onClick={() => { setSearch(''); setCategory(''); setAmc(''); setCurrentPage(1); }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {paginated.map((item) => (
            <Link
              key={item.slug}
              href={`/mutual-funds/compare/${item.slug}`}
            ...
            >
              ...
              <div className="mt-3 flex items-center gap-3 text-sm">
                <div className="flex items-center gap-1">
                  ...
                  <span className="font-medium">3Y: {item.fund1.returns_3y}%</span>
                </div>
                <span className="text-gray-300">|</span>
                <div className="flex items-center gap-1">
                  <TrendingUp className="w-3.5 h-3.5 text-green-600" />
                  <span className="font-medium">{item.fund2.returns_3y}%</span>
                </div>
              </div>
              <div className="mt-3 text-xs text-gray-500 truncate">
                {item.fund1.fund_house} vs {item.fund2.fund_house}
              </div>
            </Link>
          ))}
        </div>
        ...
        {totalPages > 1 && (
          <div className="flex justify-center gap-2 mt-10">
            <button
              ...
              className="px-4 py-2 rounded-lg border bg-white text-gray-700 disabled:opacity-50 hover:bg-gray-50"
            >
              Previous
            </button>
