...
export default function CalculatorPage({ calculator }: { calculator: any }) {
  return (
    <>
      ...
      {calculator.faq?.length > 0 && <FAQSection faq={calculator.faq} />}
      {calculator.related_calculators?.length > 0 && (
        <div className="mt-10">
          <h2 className="text-xl font-semibold mb-3">Related Calculators</h2>
          <div className="flex flex-wrap gap-2">
            {calculator.related_calculators.map((slug: string) => (
              <a key={slug} href={`/calculators/${slug}`} className="px-3 py-1 bg-gray-100 rounded hover:bg-gray-200 text-sm">{slug.replace(/-/g, ' ')}</a>
            ))}
          </div>
        </div>
      )}
